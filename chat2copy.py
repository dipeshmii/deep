...
def get_material_type():
    return st.selectbox("Material Type", ["Glass", "HDPE", "PET", "PLA", "Paperboard","Aluminum"])

def get_product_state():
    return st.selectbox("Product State", ["Liquid", "Solid","Semisolid"])

def get_storage_condition():
    return st.selectbox("Storage Condition", ["Chilled", "Refrigerated","Ambient"])

def predict_species(te, hu, material, state, condition):
    loaded_model = pickle.load(open('proj.pkl','rb'))
    material_ohe = [1 if material == m else 0 for m in ["Glass", "HDPE", "PET", "PLA", "Paperboard","Aluminum"]]
    state_ohe = [1 if state == s else 0 for s in ["Liquid", "Solid","Semisolid"]]
    condition_ohe = [1 if condition == c else 0 for c in ["Chilled", "Refrigerated","Ambient"]]
